...
<style>
    ...
    .header-layout { width: 100%; margin-bottom: 15px; border-collapse: collapse; }
    .header-layout td { vertical-align: middle; border: none; }
    .logo-cell { width: 250px; }
    .title-cell { text-align: right; }
    .pdf-logo { width: 170px; height: auto; }
    .pdf-app-title { color: #E31E24; font-size: 16pt; font-weight: bold; }
    .meta-table { width: 100%; margin-bottom: 20px; border-collapse: collapse; }
    .meta-table td { padding: 8px 10px; font-size: 9.5pt; background-color: #ffffff; border: 1px solid #939598; }
    .meta-label { color: #414042; background-color: #f2f2f2; font-weight: bold; text-transform: uppercase; font-size: 8pt; letter-spacing: 0.5px; width: 100px; }
    .section-container { margin-top: 15px; margin-bottom: 5px; }
    .section-title { color: #000000; font-size: 11pt; font-weight: bold; border-bottom: 2px solid #E31E24; padding-bottom: 2px; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px; -pdf-keep-with-next: true; }
    .content-block { margin-bottom: 10px; color: #414042; font-size: 10pt; padding-left: 2px; }
    .text-line { margin-bottom: 4px; }
    table.matrix-table { width: 100%; margin-top: 5px; margin-bottom: 12px; border-collapse: collapse; }
    table.matrix-table th { background-color: #414042; color: #ffffff; font-weight: bold; text-align: left; padding: 6px 10px; font-size: 9pt; text-transform: uppercase; letter-spacing: 0.5px; border: 1px solid #414042; }
    table.matrix-table tr { page-break-inside: avoid; }
    table.matrix-table td { border: 1px solid #939598; padding: 8px 10px; font-size: 9.5pt; vertical-align: top; color: #414042; }
    table.matrix-table tr:nth-child(even) td { background-color: #f9f9f9; }
    .table-key { font-weight: bold; color: #E31E24; width: 80px; }
    .step-img-container { margin-top: 5px; margin-bottom: 15px; text-align: left; }
    .step-img { width: 200px; height: auto; border: 1px solid #cbd5e1; }
    .step-img-caption { font-size: 8pt; color: #414042; font-style: italic; margin-top: 3px; }
    .image-grid { width: 100%; margin-top: 10px; border-collapse: collapse; }
    .image-grid tr { page-break-inside: avoid; }
    .image-grid td { width: 50%; padding: 5px; text-align: center; vertical-align: top; }
...
# ---------------------------------------------------------------------------
# IMAGE RESOLUTION HELPERS (UPDATED FOR MULTIPLE IMAGES)
# ---------------------------------------------------------------------------
...
def resolve_steps(steps, prefix):
    ...
    for i, s in enumerate(steps):
        ...
        for img_dict in s["images"]:
            img_idx = img_dict["idx"]
            step_key = f"{prefix}_{step_num}_{img_idx}"
            
            # Backward compatibility check for older drafts
            loaded_entry = st.session_state.loaded_images.get(step_key)
            if not loaded_entry and img_idx == 1:
                loaded_entry = st.session_state.loaded_images.get(f"{prefix}_{step_num}")
                
            img_res = resolve_image(img_dict["image"], loaded_entry)
            
            if img_res:
                st.session_state.loaded_images[step_key] = img_res
                resolved_images.append({"image": img_res, "caption": img_dict["caption"]})
                
        resolved.append({"text": s["text"], "images": resolved_images})
    return resolved

def imgs_html_block(resolved_images_list):
    if not resolved_images_list:
        return ""
    html_parts = []
    for img_item in resolved_images_list:
        img = img_item["image"]
        caption = img_item["caption"]
        tag = f'<img class="step-img" src="data:{img["type"]};base64,{img["b64"]}">'
        cap_html = f'<div class="step-img-caption">{caption}</div>' if caption.strip() else ""
        html_parts.append(f'<div class="step-img-container">{tag}{cap_html}</div>')
...
st.markdown("""
    <style>
        ...
        .stButton>button { background-color: #E31E24 !important; color: white !important; border-radius: 5px !important; border: none !important; font-weight: bold !important; }
...
_defaults = {
    "input_doc_title": "", "input_template_num": "", "input_date": date.today(),
    "input_author": "", "input_purpose": "", "field_1_users": "",
    "field_1_mgmt": "", "field_6_narrative": "", "field_9": "",
}
    ...
    if st.session_state.applied_draft_id != _draft_id:
        try:
            ...
            if "input_date" in values:
                try: values["input_date"] = datetime.strptime(values["input_date"], "%Y-%m-%d").date()
                except (ValueError, TypeError): values["input_date"] = date.today()
...
with tab_setup:
    ...
    with col_user: fields["field_1_users"] = st.text_area("Users:", key="field_1_users", height=80)
    with col_mgmt: fields["field_1_mgmt"] = st.text_area("Management:", key="field_1_mgmt", height=80)
...
def build_dynamic_section(title, prefix, count_key, add_label, del_label, text_label="Details"):
    ...
    for i in range(st.session_state[count_key]):
        step_num = i + 1
        # Adjusted ratio to give the image column slightly more room for side-by-side buttons
        col_txt, col_img = st.columns([2, 1.2]) 
        
        with col_txt:
            s_txt = st.text_area(f"{text_label} {step_num}:", key=f"txt_{prefix}_{step_num}", height=120)
            
        with col_img:
            # Track how many images this specific step contains
            img_count_key = f"img_count_{prefix}_{step_num}"
            if img_count_key not in st.session_state:
                st.session_state[img_count_key] = 1
                
            step_images = []
            # Loop through however many image slots this step requires
            for j in range(st.session_state[img_count_key]):
                img_idx = j + 1
                img_key = f"img_{prefix}_{step_num}_{img_idx}"
                cap_key = f"cap_{prefix}_{step_num}_{img_idx}"
                
                # --- Backward compatibility for drafts ---
                # If loading an older draft, move the old caption to the new indexed state
                if img_idx == 1 and cap_key not in st.session_state:
                    if f"cap_{prefix}_{step_num}" in st.session_state:
                        st.session_state[cap_key] = st.session_state[f"cap_{prefix}_{step_num}"]
                        
                s_img = st.file_uploader(f"Image {step_num}.{img_idx}:", type=["png", "jpg", "jpeg"], key=img_key)
                s_cap = st.text_input(f"Caption {step_num}.{img_idx}:", key=cap_key)
                
                # Retrieve saved image from session (with backward compatibility fallback)
                saved_img_data = st.session_state.loaded_images.get(f"{prefix}_{step_num}_{img_idx}")
                if not saved_img_data and img_idx == 1:
                    saved_img_data = st.session_state.loaded_images.get(f"{prefix}_{step_num}")
                    
                if s_img is None and saved_img_data:
                    st.caption("📎 Using saved image from draft")
                    
                step_images.append({"image": s_img, "caption": s_cap, "idx": img_idx})
                
                if j < st.session_state[img_count_key] - 1:
                    st.divider() # Separate multiple images visually in the UI

            # Controls to add or remove images inside THIS step
            col_add_img, col_del_img = st.columns(2)
            with col_add_img:
                if st.button("➕ Add Image", key=f"btn_add_img_{prefix}_{step_num}", use_container_width=True):
                    st.session_state[img_count_key] += 1
                    st.rerun()
            with col_del_img:
                if st.session_state[img_count_key] > 1:
                    if st.button("❌ Remove", key=f"btn_del_img_{prefix}_{step_num}", use_container_width=True):
                        st.session_state[img_count_key] -= 1
                        st.rerun()

        # Step data is now appended with a LIST of images
        steps.append({"text": s_txt, "images": step_images})
        st.divider()
...
with tab_wrapup:
    ...
    if uploaded_images:
        ...
        for idx, img in enumerate(uploaded_images):
            image_captions[img.name] = st.text_input(f"Description for figure ({img.name}):", key=f"cap_grid_{idx}_{img.name}")
...
def build_draft_payload():
    ...
    for prefix, count in active_sections:
        for i in range(count):
            step_num = i + 1
            values[f"txt_{prefix}_{step_num}"] = st.session_state.get(f"txt_{prefix}_{step_num}", "")
            
            # Save the number of images this step currently uses
            img_count_key = f"img_count_{prefix}_{step_num}"
            img_count = st.session_state.get(img_count_key, 1)
            values[img_count_key] = img_count
            
            # Iterate through those images to save their specific captions
            for j in range(img_count):
                img_idx = j + 1
                values[f"cap_{prefix}_{step_num}_{img_idx}"] = st.session_state.get(f"cap_{prefix}_{step_num}_{img_idx}", "")
...
with col_save:
    ...
    st.download_button("💾 SAVE DRAFT (.json)", data=draft_json, file_name=f"WI_Draft_{safe_title}.json", mime="application/json", use_container_width=True)
